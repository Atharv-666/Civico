...
import { 
  MapPinIcon, 
  ArrowLongRightIcon, 
  PencilSquareIcon, // Added back
  TrashIcon // Added back
...
const MyIssues = () => {
  ...
  const fetchMyIssues = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/issues/user/${user.id}`);
  ...
  // Ensure handleDelete logic is present
  ...
  if (loading) return <div className="min-h-screen bg-[#030712] flex items-center justify-center text-white text-2xl font-black italic tracking-tighter">SYNCING ACTIVITY...</div>;
    ...
    <div className="min-h-screen bg-[#030712] text-white pt-24 pb-20 px-6">
      <div className="max-w-6xl mx-auto">
        <header className="mb-12 border-b border-white/10 pb-8 text-center md:text-left flex flex-col md:flex-row justify-between items-end gap-4">
          <div>
            <h1 className="text-4xl font-black mb-3 italic uppercase font-outfit">
          ...
          <div className="bg-white/5 border border-white/10 px-6 py-2 rounded-2xl mb-2">
            ...
            <span className="text-[10px] font-bold text-gray-500 uppercase ml-2 tracking-widest font-mono">Reports</span>
          </div>
        </header>

        {issues.length === 0 ? (
          <div className="text-center py-20 bg-[#0b0f1a] rounded-[3rem] border border-dashed border-white/10">
            <p className="text-gray-500 font-medium italic">"You haven't reported any issues yet."</p>
            <Link to="/createissue" className="text-[#ff4d6d] text-sm font-bold mt-4 inline-block hover:underline">Report an Issue →</Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {issues.map((issue) => (
              <div key={issue._id} className="bg-[#0b0f1a] border border-white/10 rounded-3xl overflow-hidden flex flex-col group hover:border-[#ff4d6d]/50 transition-all relative">
                
                {/* --- ACTION BUTTONS ADDED BACK HERE --- */}
                ...
                <div className="h-48 w-full bg-black/40 overflow-hidden">
                  {issue.image ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-gray-700 text-xs font-bold uppercase tracking-tighter">No Evidence Attached</div>
                  )}
                </div>
                ...
                <div className="p-6">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-[10px] text-gray-600 font-mono">#{issue._id.slice(-4)}</span>
                  </div>
                  ...
                  <Link to={`/issues/${issue._id}`} className="w-full py-3 border border-white/5 rounded-xl flex items-center justify-center gap-2 text-xs font-bold hover:bg-[#ff4d6d] hover:text-white transition-all group/btn">
                    VIEW DETAILS
